...
fn main() {
  ...
  "provenance_limitations": [
      ...
      "KNOWN DEFECT: on a design whose SDC does not constrain its input ports, this engine can act on hold violations that do not exist. The timer behind it times a path from EVERY primary input port, seeding it from a job-level default when the SDC declares no set_input_delay for that port; OpenSTA treats such a port as unconstrained and creates no timing path from it at all. The effect is one-sided -- harmless on setup, where an unconstrained port has most of a period to spare, and large on hold, where its arrival is compared against a capture clock carrying real insertion delay and uncertainty. Async resets are the pins most often driven straight from an unconstrained port, so they dominate the result.",
      "Measured on a routed sky130 block whose SDC declares only create_clock, set_clock_uncertainty and set_propagated_clock: setup WNS 6.7322 against OpenSTA 6.85 (1.7% apart), but hold WHS -1.0682 against OpenSTA +0.88 and sign-off +0.8821 -- out by 1.95 ns and disagreeing about the sign. Asked about the endpoint this toolchain calls worst, OpenSTA answers No paths found, because the reset reaching it comes from an unconstrained input port. On that block this engine inserts 599 delay cells into a design already hold-clean by 0.88 ns.",
      "Until it is fixed: constrain your inputs, since a set_input_delay on every input port removes the cause and is good SDC practice regardless; check the worst hold endpoints before applying a plan and trace each back, because one starting at an input port with no declared arrival is not a real violation; and exclude instances with dont_touch, which accepts globs. The plan-and-apply split limits the damage -- this engine emits a plan and never mutates a design -- but the plan must be reviewed."
  ],
  "invocation": {
